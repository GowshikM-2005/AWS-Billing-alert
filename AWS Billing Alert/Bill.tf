...
resource "aws_sns_topic" "billing_alert" {
  ...
  tags = {
    Environment = lookup(var.environment_tag,terraform.workspace, "development")
...
resource "aws_cloudwatch_log_group" "billing_log_group" {
  name = "/aws/billing"
}
resource "aws_cloudwatch_log_metric_filter" "billing_logs" {
  name           = "EstimatedChargesFilter"
  log_group_name = aws_cloudwatch_log_group.billing_log_group.name
